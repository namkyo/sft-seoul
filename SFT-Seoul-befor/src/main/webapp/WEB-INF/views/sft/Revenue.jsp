	...
	<div id="ContentPlaceHolder1_UpdatePanel1" class="sect">
		...
		<ul class="boardList ntcList">
			...
			<c:forEach var="revenueList" items="${revenueList}">
				<li><span>${revenueList.num}</span>
					<div>
						<h1>
							<a href="/sft?page=Revenue_detail&&num=${revenueList.num}"><strong>${revenueList.title}</strong></a>
						</h1>
					</div>
					<span class="file"></span> <span>${revenueList.reg_date}</span></li>
			</c:forEach>
...
.paging a{padding-right:2%;}
		...
		<div class="pagination paging">
			...
			<c:if test="${pageMaker.prev}">
				<b><a class="next1"
					href="/sft?page=Revenue&pageNum=${pageMaker.startPage-1 }">이전</a></b>
			</c:if>
			...
			<c:forEach begin="${pageMaker.startPage }"
				...
				<li>
					<a href="/sft?page=Revenue&pageNum=${idx}">${idx}</a>
				</li>
				<c:if test="${pageMaker.cri.page == idx}">
				</strongs>
				</c:if>
			...
			<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
				<b><a class="next1"
					href="/sft?page=Revenue&pageNum=${pageMaker.endPage+1} ">다음</a>
				</b>
			</c:if>
